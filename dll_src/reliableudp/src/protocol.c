...
#ifdef USE_CRC32
	if(header->flags & ENET_PROTOCOL_HEADER_FLAG_CRC)
    {
...
enet_protocol_handle_incoming_commands (ENetHost * host, ENetEvent * event)
{
	...
	peerID &= ~ ENET_PROTOCOL_HEADER_PEER_MASK;
	...
	if(flags & ENET_PROTOCOL_HEADER_FLAG_SENT_TIME)
		printf("PACKET FLAGGED: %x %d %d %d\n", flags, headerSize, sizeof(ENetProtocolHeader), (size_t) & ((ENetProtocolHeader *) 0) -> sentTime);
	else
		printf("PACKET NOT FLAGGED: %x %d %d %d\n", flags, headerSize, sizeof(ENetProtocolHeader), (size_t) & ((ENetProtocolHeader *) 0) -> sentTime);
#endif
...
#ifdef USE_CRC32
		if(currentPeer -> lastSendTime % 2) {
			//printf("Sending CRC\n");
			header.flags |= ENET_PROTOCOL_HEADER_FLAG_CRC;
...
enet_host_service (ENetHost * host, ENetEvent * event, enet_uint32 timeout)
{
	...
	int result = enet_socket_wait (host -> socket, & waitCondition, ENET_TIME_DIFFERENCE (timeout, host -> serviceTime));
	if (result == -2) // INTR
		return 0;
	if (result != 0) {
		//perror("Socket wait error");
		return -1;
	}
